...
async def deptrai_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        ...
        await ctx.send(f"⚠️ Cậu spam lệnh nhanh quá! Hãy chờ **{s} giây** nữa rồi dùng lại nhé. 🍵", delete_after=10)

# --- LỆNH MHELP (Cooldown 60 giây) ---
...
@monika_help.error
async def monika_help_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        s = int(error.retry_after)
        await ctx.send(f"⚠️ Cậu spam lệnh nhanh quá! Hãy chờ **{s} giây** nữa rồi dùng lại nhé. 🍵", delete_after=10)
...
async def on_message(message):
    ...
    # AI Chat không cooldown để trò chuyện mượt mà
