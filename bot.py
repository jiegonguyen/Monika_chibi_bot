#!/usr/bin/env python3
"""
bot.py - Monika AI (chat) + minigames + voice music + soft-NSFW per-guild opt-in

Env vars:
 - DISCORD_BOT_TOKEN (required)
 - GEMINI_API_KEY (optional) - if present, Monika will call Gemini via REST
 - GEMINI_MODEL (optional) - default "gemini-2.5-flash"

Files created/used:
 - nsfw_settings.json  (per-guild NSFW enable/disable)
 - nsfw_attempts.log   (append-only log of NSFW attempts)

Dependencies (requirements.txt):
discord.py, python-dotenv, aiohttp, yt-dlp, PyNaCl, requests
"""
import os
import re
import json
...
def save_nsfw_settings(settings: dict):
    SETTINGS_PATH.write_text(json.dumps(settings, ensure_ascii=False, indent=2), encoding="utf-8")

nsfw_settings = load_nsfw_settings()  # maps guild_id (str) -> {"enabled": bool}
...
# ---------------- Utilities: clean / programming words / explicit filter ----------------
PROG_TOKENS = [
...
EXPLICIT_TOKENS = [
    # Vietnamese and English suggestive/explicit tokens (extend as needed)
    "sex", "tình dục", "quan hệ", "âm đạo", "dương vật", "bú", "thổi", "oral", "anal",
    "fuck", "suck", "porn", "pussy", "penis", "vagina", "masturb", "rape", "sexually",
    "ngực", "vú", "lồn", "địt", "zịt"
]
...
# ---------------- Monika AI Engine (REST async + fallback + nsfw mode) ----------------
class MonikaAIEngine:
    def __init__(self, api_key: Optional[str] = None, model: str = "gemini-2.5-flash"):
        ...
        self.recent_replies: dict[int, deque] = {}  # per-user recent reply history

        self.system_instruction = (
            "Bạn là Monika: dịu dàng, sâu sắc, thân mật nhưng lịch sự. Trả lời bằng tiếng Việt, ngắn gọn (1-3 câu), "
            "giàu cảm xúc và ân cần. Tránh nội dung NSFW rõ rệt, bạo lực hoặc bất hợp pháp."
        )
        self.fallback_openings = ["Mmm...", "Ồ...", "Ừm...", "Hehe...", "Aha...", "Hừm..."]
        self.fallback_mids = ["mình thích điều đó", "mình muốn nghe thêm", "mình cảm thấy nhẹ lòng", "cậu làm mình mỉm cười"]
    ...
    async def generate_response(self, user_message: str, user_id: Optional[int] = None, allow_nsfw: bool = False) -> str:
        """
        allow_nsfw: if True, model is allowed to use suggestive/joke tone but must NOT produce explicit sexual descriptions.
        """
        if self.enabled:
            try:
                ...
                if allow_nsfw:
                    persona += (" Bạn được phép dùng ngôn từ gợi ý, dí dỏm, tán tỉnh nhẹ nhàng (joke/flirt), "
                                ...
                                "không liên quan trẻ vị thành niên và không khuyến khích bạo lực hay trái pháp luật.")
                ...
                # Extract candidate from various shapes
                candidate = None
                ...
                # Safety: if contains explicit despite allow_nsfw, refuse and log
                if contains_explicit(candidate):
                    # log attempt
                    dummy_interaction = None  # will be logged by caller where we have interaction; here we just sanitize
                    # return polite refusal
                    return "Mình không thể mô tả chi tiết như vậy. Nếu muốn, chúng ta có thể đùa nhẹ hoặc nói với cách tế nhị hơn."
...
# ---------------- Music/Voice support (yt-dlp + ffmpeg) ----------------
YTDL_OPTS = {
...
# Hangman/Trivia/RPS/CoinFlip/Scramble/MathQuiz definitions (same as earlier merged code)
...
# Helper: is NSFW allowed in context
...
def is_nsfw_allowed_for_interaction(interaction: discord.Interaction) -> bool:
    ...
    enabled = nsfw_settings.get(guild_id, {}).get("enabled", False)
    ch_nsfw = channel_is_nsfw(interaction.channel)
    return enabled and ch_nsfw

# NSFW admin command
@bot.tree.command(name="nsfw", description="Quản lý soft-NSFW cho server (admin only)")
...
async def nsfw_toggle(interaction: discord.Interaction, action: str):
    ...
    if action in ("enable", "on"):
        ...
        await interaction.response.send_message("Đã bật chế độ soft-NSFW cho server. Chỉ hoạt động trên channel NSFW hoặc DM.", ephemeral=False)
...
# Chat command: include allow_nsfw based on context, log attempts
...
async def chat(interaction: discord.Interaction, message: str):
    ...
    try:
        ...
        reply = "Ừm... tớ hơi chậm, thử gửi lại nhé."
    except Exception as e:
        print("AI error:", e)
        traceback.print_exc()
        reply = "Có chút vấn đề khi gọi AI, nhưng tớ vẫn ở đây nhé."
    # If allow_nsfw was True, check explicit content in reply or in user message
    if allow_nsfw and (contains_explicit(message) or contains_explicit(reply)):
        # log attempt (including generated)
        log_nsfw_attempt(interaction, message, reply)
        # sanitize / refuse
        reply = "Mình không thể mô tả chi tiết như vậy. Nếu muốn, chúng ta có thể đùa nhẹ hoặc nói tế nhị hơn."
    reply = clean_output(reply)
    await interaction.followup.send(f"**{interaction.user.name}:** {message}\n\n💬 **Monika:** {reply}")

# Music / voice commands (join/leave/play/skip/stop/now)
...
async def join(interaction: discord.Interaction):
    ...
    try:
        ...
        print("Join error:", e)
        traceback.print_exc()
        await interaction.followup.send(f"Không thể kết nối: {e}", ephemeral=True)
...
async def play(interaction: discord.Interaction, query: str):
    ...
    if not voice or not voice.is_connected():
        ...
        try:
            ...
            traceback.print_exc()
            await interaction.followup.send("Không thể kết nối voice.", ephemeral=True)
            return
    else:
        await interaction.response.defer()
    try:
        ...
        print("YTDL error:", e)
        traceback.print_exc()
        await interaction.followup.send("Không tải được media, thử URL khác.", ephemeral=True)
...
# Health server (for Render Web Service) - optional
