#!/usr/bin/env python3
"""
Monika chibi bot - complete file

Features:
 - Gemini-based AI replies (tries Authorization: Bearer KEY then ?key=KEY)
 - Points/Level system:
    * Chat: +5 pts, cooldown 5s
    * Minigames: +50 pts, cooldown 30s
    * Daily cap default 500, increased by items and by level (+5% per level)
    * Level every 100 pts; when leveling up user receives +300 pts (one-time)
 - Shop /store /inventory
    * Items: small_boost, large_boost, daily_plus, doki_chest
    * /shop (buttons), /store to sell (50% refund), /inventory to view
    * /use to consume items (activate boosts, open chests)
 - Doki Chest gacha (random points/items)
 - Cooldowns to avoid spam
 - NSFW soft session handling
 - /diag_ai admin command to test Gemini connectivity
 - Persistence via JSON files: points.json, inventory.json, effects.json, nsfw_settings.json
 - Health server for hosting checks
"""
import os
import re
import json
...
# ---------------- Monika AI Engine (improved: try Bearer then ?key) ----------------
class MonikaAIEngine:
    ...
    async def _call_generative_api(self, url: str, headers: Dict[str, str], body: Dict) -> Tuple[int, str]:
        try:
    ...
    async def generate_response(self, user_message: str, user_id: Optional[int] = None, allow_nsfw: bool = False) -> str:
        ...
        if allow_nsfw:
            persona += (" Bạn được phép dùng ngôn từ gợi ý, dí dỏm, tán tỉnh nhẹ nhàng (joke/flirt), "
                        ...
                        "không liên quan trẻ vị thành niên và không khuyến khích bạo lực.")
        prompt_text = f"{persona}\n\nNgười dùng: {user_message}\nMonika:"
        body = {"prompt": {"text": prompt_text}, "temperature": 0.75, "maxOutputTokens": 300}

        base_url = f"https://generative.googleapis.com/v1/models/{self.model}:generate"

        # Attempt 1: Authorization: Bearer {KEY}
        headers = {"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json"}
        status, text = await self._call_generative_api(base_url, headers, body)

        print(f"[GenAPI] Attempt 1 (Bearer) status={status}")
        if isinstance(text, str):
            print(f"[GenAPI] Attempt 1 body (trim): {text[:800]}")
        ...
        if status == 200:
            ...
            if isinstance(data, dict):
                ...
                if not candidate and "text" in data and isinstance(data["text"], str):
                    candidate = data["text"]
            if not candidate:
                candidate = clean_output(str(data)[:1000])
        ...
        # If unauthorized-like, try fallback with ?key=API_KEY
        if status in (401, 403):
            safe_key = self.api_key
            url_with_key = f"{base_url}?key={safe_key}"
            headers2 = {"Content-Type": "application/json"}
            status2, text2 = await self._call_generative_api(url_with_key, headers2, body)
            print(f"[GenAPI] Attempt 2 (?key) status={status2}")
            if isinstance(text2, str):
                print(f"[GenAPI] Attempt 2 body (trim): {text2[:800]}")
            ...
            if status2 == 200:
                ...
                if contains_explicit(candidate):
                    return "Xin lỗi — nội dung yêu cầu không phù hợp. Mình không thể mô tả chi tiết như vậy."
                return self._postprocess(candidate, user_id)
            print(f"[GenAPI] Both attempts failed: status1={status}, status2={status2}")
            return "Xin lỗi — Monika hiện đang gặp sự cố khi gọi AI. Hãy thử lại sau."

        print(f"[GenAPI] Non-auth error on first attempt: status={status}")
        return "Xin lỗi — Monika hiện đang gặp sự cố khi gọi AI. Hãy thử lại sau."
...
def add_points(user_id: int, base_amt: int) -> Tuple[int, bool, int]:
    ...
    if new_level > old_level:
        ...
        # recompute final level after adding bonus (in case bonus pushed more levels)
