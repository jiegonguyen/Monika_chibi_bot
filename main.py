...
# 3. Cấu hình Google Gemini AI với gemini-3.6-flash
GOOGLE_API_KEY = os.environ.get("GOOGLE_API_KEY")
if GOOGLE_API_KEY:
    genai.configure(api_key=GOOGLE_API_KEY)
    ai_model = genai.GenerativeModel(model_name="gemini-3.6-flash")
else:
...
# --- CÁC LỆNH MINIGAME ---
@bot.command(name='roll', help='Tung xúc xắc ngẫu nhiên từ 1 đến 6.')
async def roll_dice(ctx):
    number = random.randint(1, 6)
    await ctx.send(f"🎲 Kết quả tung xúc xắc của **{ctx.author.display_name}** ra số: **{number}**!")

@bot.command(name='guess', help='Đoán số từ 1 đến 10.')
async def guess_number(ctx, number: int):
    target = random.randint(1, 10)
    if number == target:
        await ctx.send(f"🎉 Chúc mừng **{ctx.author.display_name}**! Cậu đã đoán trúng số **{target}**!")
    else:
        await ctx.send(f"❌ Tiếc quá, số may mắn là **{target}** cơ!")
...
async def on_message(message):
    ...
    if bot.user.mentioned_in(message) and not message.content.startswith('!'):
        if not GOOGLE_API_KEY:
            await message.channel.send("⚠️ Chưa thiết lập Google API Key.")
            return

        user_prompt = message.content.replace(f'<@!{bot.user.id}>', '').replace(f'<@{bot.user.id}>', '').strip()
        if not user_prompt:
            await message.channel.send("Cậu gọi tớ đấy à? 💚")
            return
