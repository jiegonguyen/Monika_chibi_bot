...
# --- HÀM GỌI GEMINI TRỰC TIẾP QUA HTTP REQUEST (TRIỆT TIÊU LỖI V1BETA) ---
def ask_gemini(prompt_text):
    if not GOOGLE_API_KEY:
        return "⚠️ E hèm... Cậu chưa thiết lập Google API Key trên Render kìa! 💚"
    
    # Dùng v1 (endpoint ổn định nhất) với model gemini-1.5-flash
    url = f"https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash:generateContent?key={GOOGLE_API_KEY}"
    
    headers = {'Content-Type': 'application/json'}
    
    # Định hình nhân vật Monika
    full_prompt = (
    ...
    try:
        ...
        # Bóc tách kết quả trả về từ cấu trúc JSON của Google API
